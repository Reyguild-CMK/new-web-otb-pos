          ...
          const updatedList = state.transactionList.map(tx => {
            if (tx.id === state.activeTransactionId) {
              return {
                ...
                customerId: state.selectedCustomer ? Number(state.selectedCustomer) : tx.customerId,
                invoice: state.pawnDocs?.bukti_transaksi || tx.invoice,
                suratsegel: state.pawnDocs?.surat_segel_barang || tx.suratsegel,
